...
export function BlogNav({ active }: Props) {
  ...
  return (
    <>
      <nav className="fixed left-0 right-0 top-0 z-[100] flex w-full max-w-[100vw] flex-nowrap items-center gap-2 border-b border-white/[0.08] bg-[rgba(6,9,16,0.92)] px-[max(1rem,4%)] py-3 backdrop-blur-[16px] pl-[max(1rem,calc(4%+env(safe-area-inset-left)))] pr-[max(1rem,calc(4%+env(safe-area-inset-right)))] pt-[max(0.75rem,calc(0.75rem+env(safe-area-inset-top)))] sm:gap-3 sm:py-4">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          ...
          <Link
            href="/#apply"
            className="inline-flex items-center gap-1 rounded-md bg-gradient-to-br from-[#6ba3f5] to-[#2a52c0] px-3 py-2 text-[0.8rem] font-bold text-[#f8fafc] shadow-[0_2px_0_rgba(0,0,0,0.18),0_6px_20px_rgba(42,82,192,0.35)] no-underline transition-all hover:brightness-110 sm:px-[1.4rem] sm:py-[0.6rem] sm:text-[0.875rem]"
          >
            Apply Now →
          </Link>
        </div>
